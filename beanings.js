...
const packCostbean = 7500000; // Cost to open a bean+ pack
let selectedPlayersbean = []; // Array to hold currently selected players
...
const quickSellValuesbeanPack = {
  hawkjamal: 2500000,
  talktuah: 5000000,
  fillyfiddler: 2500000,
  davidspectrum: 2500000,
  clarksnort: 2500000,
  youngman: 2500000,
...
// Function to open the bean+ pack
function openbeanPack() {
  if (coins < packCostbean) {
    alert("Not enough coins to open a Beanings pack!");
    return;
  }
  ...
  document.getElementById("openbeanPackButton").style.display = "none";
  document.getElementById("currentPlayer").innerHTML = "";
  document.getElementById("inventoryButton").style.display = "none";

  selectedPlayersbean = shuffleArray(Object.keys(quickSellValuesbeanPack)).slice(
    0,
    3
  );
  selectedPlayersbean.sort(
    (a, b) => (quickSellValuesbeanPack[b] || 0) - (quickSellValuesbeanPack[a] || 0)
  );

  const firstPlayer = selectedPlayersbean[0];
...
function showAllPlayers() {
  ...
  selectedPlayersbean.forEach((playerId) => {
...
function quickSellPlayer(player) {
  const sellValue = quickSellValuesbeanPack[player.id] || 0;
  coins += sellValue;
  updateCoinBalance();

  selectedPlayersbean = selectedPlayersbean.filter((p) => p !== player.id);
  ...
  if (selectedPlayersbean.length === 0) {
    document.getElementById("sendToInventoryButton").style.display = "none";
    document.getElementById("openbeanPackButton").style.display = "inline-block"; // Show Open Pack button
  }
}
...
function sendToInventory() {
  ...
  selectedPlayersbean = []; // Clear selected players
  ...
  document.getElementById("openbeanPackButton").style.display = "inline-block"; // Ensure the Open Pack button is visible
}
...
// Event Listeners
document
  .getElementById("openbeanPackButton")
  .addEventListener("click", openbeanPack);
document
